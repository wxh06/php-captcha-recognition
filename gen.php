...
function distort($image, $width, $height, $bg)
{
    ...
    for ($x = 0; $x < $width; $x++) {
        for ($y = 0; $y < $height; $y++) {
            ...
            $p  = interpolate($nX - floor($nX), $nY - floor($nY), getCol($image, floor($nX), floor($nY), $bg), getCol($image, ceil($nX), floor($nY), $bg), getCol($image, floor($nX), ceil($nY), $bg), getCol($image, ceil($nX), ceil($nY), $bg));
